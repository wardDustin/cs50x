...
char* indexes(const char* path)
{
    //check for a php file, allocates enough space and then catenates it onto a copy of path (since path is a const)
    char* php = "index.php";
    char* newpath = malloc(sizeof(char) * strlen(path) + strlen(php) +1);
    strcpy(newpath, path);
    strcat(newpath, php);
    //access checks if the php file can be... accessed and F_OK confirms the existence
    if (access(newpath, F_OK)==0)
        return newpath;
    
    //check for an html file, allocates enough space and then catenates it onto a copy of path (since path is a const)
    char* html = "index.html";
    char* newpath2 = malloc(sizeof(char) * strlen(path) + strlen(html) +1);
    strcpy(newpath2, path);
    strcat(newpath2, html);
    //access checks if the html file can be... accessed and F_OK confirms the existence
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    //initialized a counter for the bytes
    int byteCounter=0;
    //creates a small buffer to hold the bytes
    char* buffer=malloc(sizeof(BYTE));
    //iterates through the file
    while(fread((buffer + byteCounter),1,1,file)>0){
        //keeps track of the bytes and reallocs memory for enough space
        byteCounter++;
        buffer = realloc(buffer, byteCounter+1);
    }
    //derefrences length and stores the counter
    *length=byteCounter;
    //derefrences content and stores the file contents
    *content=buffer;
...
const char* lookup(const char* path)
{
    //pointer to hold the constant path
    char* actual = NULL;
    //looks for the last instance of a period and gives back a pointer to the extension (and error checks)
    actual = strrchr(path, '.');
    if (actual==NULL)
        return NULL;
    //checks for the extension and returns the correct output
    if (strcasecmp(actual, ".css")==0){
        return "text/css";
    }
    else if (strcasecmp(actual, ".html")==0){
        return "text/html";
    }
    else if (strcasecmp(actual, ".gif")==0){
        return "image/gif";
    }
    else if (strcasecmp(actual, ".ico")==0){
        return "image/x-icon";
    }
    else if (strcasecmp(actual, ".jpg")==0){
        return "image/jpeg";
    }
    else if (strcasecmp(actual, ".js")==0){
        return "text/javascript";
    }
    else if (strcasecmp(actual, ".php")==0){
        return "text/x-php";
    }
    else if (strcasecmp(actual, ".png")==0){
        return "image/png";
    }
    else{
        return NULL;
...
{
    //initializing pointers to NULL to later point to strings of request line
    char* method = NULL;
    char* target = NULL;
    char* version = NULL;
    //temp copies line which is a constant and a request line that needs to be parsed
    //temp2 will hold values after temp has been manipulated
    char* temp = malloc(sizeof(char)*strlen(line)+2);
    char* temp2 = malloc(sizeof(char)*strlen(line)+2);
    strcpy(temp, line);
    
    //method is the command, target is the destination and version is the protocol of the request line
    method = strsep(&temp, " ");
    target = strsep(&temp, " ");
    version = strsep(&temp, "\r\n");

    //a series of if statements to make sure the request-line is formatted correctly
    ...
    //temp now holds a pointer starting with the ? (unless it is null)
    temp = strchr(target, '?');
    //if temp holds the pointer, copy it to query at the pointer plus one, if it is null, consume one byte
    if (temp!=NULL)
        strcpy(query, temp+1);
    else
        query="";
    //null terminates target (strcpy returns non-null-terminated string)
    target[strlen(target)]='\0';
    //temp2's time to shine... prepares for abs_path by copying target
    memcpy(temp2, target, strlen(target)+1);
    //sheds the ?query
    temp2 = strsep(&temp2, "?");
    //sets abs_path
    memcpy(abs_path, temp2, strlen(temp2)+1);
    
    // free(temp);
    free(temp2);
    
    //line is formatted correctly and has been parsed
    return true;
}
